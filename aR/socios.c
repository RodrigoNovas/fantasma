...
int socio_add(eSocio* socios,int cantidad)
{
    ...
    if(cantidad>=0)
    {
        ...
        if(posicionAux!=-1)
        {
            ...
            socios[posicionAux].telefono= getValidInt("INGRESE NUMERO DE TELEFONO\n","ERROR\nCARACTER NO VALIDO");
            socios[posicionAux].idUnico= socio_array_generarProximoId();
            system("cls");
            getValidString("\tINGRESE SEXO\n(F)FEMENINO\n(M)MASCULINO\n","\tERROR\nCARACTER NO VALIDO\n",socios[posicionAux].sexo);
            getValidString("INGRESE EMAIL:\n","ERROR",socios[posicionAux].email);
            system("cls");
            socios[posicionAux].fechaAsociado.anio= getValidIntOpcion("INGRESE ANIO DE INGRESO\n ","\tERROR\nOPCION NO VALIDA",1950,2019);
            socios[posicionAux].fechaAsociado.mes= getValidIntOpcion("INGRESE MES DE INGRESO\n ","\tERROR\nOPCION NO VALIDA",1,12);
            socios[posicionAux].fechaAsociado.dia= getValidIntOpcion("INGRESE DIA DE INGRESO\n ","\tERROR\nOPCION NO VALIDA",1,31);
            system("cls");
...
void socio_hardcodearDatos(eSocio* socios , int cantidad)
{
    ...
    int telefono[]={5551234,5551235,5551236,5551237,5551238,5551239};
    char email[][50]= {"josue@gmail.com","maria@gmail.com","carlos@gmail.com","pedro@gmail.com","sofia@gmail.com","mateo@gmail.com"};
...
int socio_modificar(eSocio* socios, int cantidad,int id)
{
    ...
    if(idAux!= -1)
    {
        do
        {
            ...
            case 6:
                system("cls");
                getValidStringAlfaNumerico("INGRESE EMAIL\n","ERROR\nEMAIL NO VALIDO",socios[idAux].email);
                break;
            }
        }
        while(opcion!=8);
